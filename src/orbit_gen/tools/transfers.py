...
class transfer_orbits:
    ...
    def lambert(self, r1, r2, a):
        ...
        nu = math.acos(
            (x1 * x2 + y2 * y2) / (r1mag * r2mag)
        )  # angle between arrival and departure vectors
        ...
        R1 = 2 * a - math.sqrt(
            x1**2 + y1**2
        )  # radius of intersecting circle centered at P1
        R2 = 2 * a - math.sqrt(
            x2**2 + y2**2
        )  # radius of intersecting circle centered at P2

        R = math.sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2)

        # make pre-calculaitons for faster speed
        pre_calc1 = (R1**2 - R2**2) / (2 * R**2)
        pre_calc2 = (1 / 2) * math.sqrt(
        ...
        )  # time to intercept for TO1, short way
        T2 = (Tp / (2 * pi)) * (
            (alpha - math.sin(alpha)) + (beta - math.sin(beta))
        )  # time to intercept for T01, long way

        if x_intersect1 >= 0:
            phi1 = math.atan(
                y_intersect1 / x_intersect1
            )  # angle for tilted elliptical TO1
        else:
            phi1 = pi + math.atan(y_intersect1 / x_intersect1)

        if x_intersect2 >= 0:
            phi2 = math.atan(
                y_intersect2 / x_intersect2
            )  # angle for tilted elliptical TO2
        else:
    ...
    def orbit_plot(self, orbits):
        ...
        for i in range(0, len(orbits)):
            if i < 2:
                ...
                plt.plot(
                    orbits[i][0], orbits[i][1], linestyle="--"
                )  # plot transfer orbit

        plt.xlabel("x-position [m]")
        plt.ylabel("y-position [m]")
        plt.legend(
            ["Initial orbit", "Orbit of target", "Transfer orbit"], loc="upper right"
        )
