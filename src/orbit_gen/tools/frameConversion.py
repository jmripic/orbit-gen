...
def rotMatAxisAng(n_vec, theta):
    ...
    r_skew = np.array(
        [[0, -n_hat[2], n_hat[1]], [n_hat[2], 0, -n_hat[0]], [-n_hat[1], n_hat[0], 0]]
    )
...
def gcrs2gmec(pos, currentTime, vel=None):
    ...
    if vel is not None:
        ...
        state_gmec = state_gcrs.transform_to(
            GeocentricMeanEcliptic(obstime=currentTime)
        )
        pos_gmec = state_gmec.cartesian.get_xyz()
        vel_gmec = state_gmec.velocity.get_d_xyz()
        ...
        pos_gmec = pos_gcrs.transform_to(
            GeocentricMeanEcliptic(obstime=currentTime)
        ).cartesian.get_xyz()
        return pos_gmec
...
def convertSC_I2H(pos_I, vel_I, currentTime, C_I2G, Tp_can=None):
    ...
    posEMB = (
        get_body_barycentric_posvel("Earth-Moon-Barycenter", currentTime)[0]
        .get_xyz()
        .to("AU")
    )
    velEMB = (
        get_body_barycentric_posvel("Earth-Moon-Barycenter", currentTime)[1]
        .get_xyz()
        .to("AU/d")
    )
...
def convertSC_R2I(t_start, t_current, C_I2G, state, mu_star):
    ...
    theta = np.arccos(
        (np.dot(moon_I_can, ideal_moon_I))
        / (np.linalg.norm(moon_I_can) * np.linalg.norm(ideal_moon_I))
    )
    if theta > np.pi / 2:
        theta = -theta
