...
def state_iterate(target_state, tolerance, stepsize, T_final):
    ...
    if not os.path.isfile(FileName_threeBodyInt):
        ...
        threeBodyInt = STMint(
            variables, dynamics, variational_order=1
        )  # this is the expensive line

        with open(
            "EnergyOptimal_threeBodyInt_EarthMoon_L2nrho.pickle", "wb"
        ) as threeBodyInt_file:
            dill.dump(threeBodyInt, threeBodyInt_file)

    with open(
        "EnergyOptimal_threeBodyInt_EarthMoon_L2nrho.pickle", "rb"
    ) as threeBodyInt_file:
    ...
    while (
        la.norm(la.inv(STM[:6, :6] - np.identity(6)) @ residual) > tolerance and i < 20
    ):
        [state_output, STM] = threeBodyInt.dynVar_int(
            [0, T_final],
            np.reshape(np.vstack((state, costates, np.array([0]))), (1, 13)),
            output="final",
            max_step=stepsize,
            rtol=1e-12,
            atol=1e-12,
...
def true_cost(target_state, u_inherent, tolerance, stepsize, T_final, costate_guess):
    ...
    while la.norm(la.inv(STM[:6, 6:12]) @ residual) > tolerance and i < 20:
        [state, STM] = threeBodyInt.dynVar_int(
            [0, T_final],
            np.reshape(
                np.vstack((target_state, costates_iterate, np.array([0]))), (1, 13)
            ),
            output="final",
            max_step=stepsize,
        )
...
ics = [
    ...
    0,
    0,
]  # (use this for _L2nrho)
T_final = 2.085034838884136
exponent = 8
t_step = T_final / 2.0**exponent
...
if not os.path.isfile(FileName_time):
    ...
    [state, STM, STT, time] = threeBodyInt.dynVar_int2(
        [0, T_final], ics, output="all", max_step=t_step
    )
...
STM_full = list(scipy.io.loadmat(FileName_STM).values())[
    -1
]  # state transition matrix at every time
...
if not os.path.isfile(FileName_state):
    ...
    [state, STM, STT, time] = threeBodyInt.dynVar_int2(
        [0, T_final], ics_halfway, output="all", max_step=t_step
    )
...
STM_full_halfway = list(scipy.io.loadmat(FileName_STM).values())[
    -1
]  # state transition matrix at every time
...
for i in range(0, rows):
    ...
    Matrix1_halfway = np.block(
        [
            [np.identity(6), np.zeros((6, 6))],
            [
                -la.solve(STM_halfway[:6, 6:12], STM_halfway[:6, :6]),
                la.inv(STM_halfway[:6, 6:12]),
            ],
        ]
    )
    ...
    TempMatrix1 = (
        la.inv(np.transpose(STM_full[i, :12, :12]))
        @ (STT_full[-1, 12, :12, :12] - STT_full[i, 12, :12, :12])
        @ la.inv(STM_full[i, :12, :12])
    )
    TempMatrix2 = (
        np.transpose(STM_full[-1, :12, :12] @ la.inv(STM_full[i, :12, :12]))
        @ STT_full[i, 12, :12, :12]
        @ (STM_full[-1, :12, :12] @ la.inv(STM_full[i, :12, :12]))
        + TempMatrix1
    )
    Matrix2 = TempMatrix2[:12, :12]

    TempMatrix1_halfway = (
        la.inv(np.transpose(STM_full_halfway[i, :12, :12]))
        @ (STT_full_halfway[-1, 12, :12, :12] - STT_full_halfway[i, 12, :12, :12])
        @ la.inv(STM_full_halfway[i, :12, :12])
    )
    TempMatrix2_halfway = (
        np.transpose(
            STM_full_halfway[-1, :12, :12] @ la.inv(STM_full_halfway[i, :12, :12])
        )
        @ STT_full_halfway[i, 12, :12, :12]
        @ (STM_full_halfway[-1, :12, :12] @ la.inv(STM_full_halfway[i, :12, :12]))
        + TempMatrix1_halfway
    )
    ...
    E_star = (
        np.block([[np.identity(6), np.identity(6)]])
        @ E
        @ np.transpose(np.block([[np.identity(6), np.identity(6)]]))
    )  # establish Estar matrix
    gamma, w = eigh(E_star)  # get eigenvalues and eigenvectors of E_star

    E_halfway = (
        np.transpose(Matrix1_halfway) @ Matrix2_halfway @ Matrix1_halfway
    )  # establish E-matrix
    E_star_halfway = (
        np.block([[np.identity(6), np.identity(6)]])
        @ E_halfway
    ...
    gamma_halfway, w_halfway = eigh(
        E_star_halfway
    )  # get eigenvalues and eigenvectors of E_star
    ...
    Qprime = np.transpose(
        w_adj
    )  # establish Q matrix with row-vectors corresponding to eigenvectors
    Eprime = np.diag(gamma_adj)  # form adjusted eigenvalue matrix

    w_adj_halfway = w_halfway[
        :, 1:6
    ]  # remove first eigenvector (the one with infinite extent)
    gamma_adj_halfway = gamma_halfway[
        1:6
    ]  # remove first eigenvalue (the one with infinite extent)
    Qprime_halfway = np.transpose(
        w_adj_halfway
    )  # establish Q matrix with row-vectors corresponding to eigenvectors
    Eprime_halfway = np.diag(gamma_adj_halfway)  # form adjusted eigenvalue matrix

    lam, z = eigh(
        Qprime @ np.transpose(A) @ A @ np.transpose(Qprime), Eprime
    )  # solve adjusted problem for new eigenvalues and eigenvectors
    lamyz, zyz = eigh(
        Qprime @ np.transpose(Ayz) @ Ayz @ np.transpose(Qprime), Eprime
    )  # solve adjusted problem for new eigenvalues and eigenvectors

    lam_halfway, z_halfway = eigh(
        Qprime_halfway @ np.transpose(A) @ A @ np.transpose(Qprime_halfway),
        Eprime_halfway,
    )  # solve adjusted problem for new eigenvalues and eigenvectors
    lamyz_halfway, zyz_halfway = eigh(
        Qprime_halfway @ np.transpose(Ayz) @ Ayz @ np.transpose(Qprime_halfway),
        Eprime_halfway,
    )  # solve adjusted problem for new eigenvalues and eigenvectors
    ...
    angle_halfway = np.arctan2(
        dir1_halfway[1], dir1_halfway[0]
    )  # radians, angle of ellipse rotation
    ...
    angleyz_halfway = np.arctan2(
        diryz1_halfway[1], diryz1_halfway[0]
    )  # radians, angle of ellipse rotation
    ...
    ext1_halfway = 2 * np.sqrt(
        2 * J_max * lam_halfway[3]
    )  #  width of ellipse projection
    ext2_halfway = 2 * np.sqrt(
        2 * J_max * lam_halfway[4]
    )  # height of ellipse projection
    extyz1_halfway = 2 * np.sqrt(
        2 * J_max * lamyz_halfway[3]
    )  #  width of ellipse projection
    extyz2_halfway = 2 * np.sqrt(
        2 * J_max * lamyz_halfway[4]
    )  # height of ellipse projection
    ...
    position_halfway = state_full_halfway[
        i, 0:3
    ]  # center of ellipse for current timestep
    pos_halfway[:, i] = np.transpose(state_full_halfway[i, 0:2])

    plotangle = angle
    tipsright[:, i] = xypos[:, i] + np.array(
        [
            ext2 / 2 * np.cos(plotangle - np.pi / 2),
            ext2 / 2 * np.sin(plotangle - np.pi / 2),
        ]
    )
    tipsleft[:, i] = xypos[:, i] - np.array(
        [
            ext2 / 2 * np.cos(plotangle - np.pi / 2),
            ext2 / 2 * np.sin(plotangle - np.pi / 2),
        ]
    ...
    tipsright_halfway[:, i] = xypos_halfway[:, i] + np.array(
        [
            ext2_halfway / 2 * np.cos(plotangle_halfway - np.pi / 2),
            ext2_halfway / 2 * np.sin(plotangle_halfway - np.pi / 2),
        ]
    )
    tipsleft_halfway[:, i] = xypos_halfway[:, i] - np.array(
        [
            ext2_halfway / 2 * np.cos(plotangle_halfway - np.pi / 2),
            ext2_halfway / 2 * np.sin(plotangle_halfway - np.pi / 2),
        ]
    ...
    if i % 10 == 0:
        if i == 0:
            ellipse = matplotlib.patches.Ellipse(
                ...
                width=ext1,
                height=ext2,
                edgecolor=[252 / 255, 227 / 255, 3 / 255],
                fc="r",
                angle=rotation,
                linewidth=0,
            )
...
center_halfway = np.array(
    [np.average(state_full_halfway[:, 0]), np.average(state_full_halfway[:, 1])]
)

for i in range(0, rows):
    if la.norm(tipsright_halfway[:, i] - center_halfway) > la.norm(
        pos_halfway[:, i] - center_halfway
    ):
...
(ref_trajectory,) = ax.plot(
    state_full[:, 0], state_full[:, 1], color=[0, 0, 0], label="Reference Trajectory"
)
...
(bounds,) = ax.plot(
    tipsoutside[0, :],
    tipsoutside[1, :],
    label="Bounds from Start Point 1",
    color="b",
    linewidth=3,
)
(bounds_halfway,) = ax.plot(
    tipsoutside_halfway[0, :],
    tipsoutside_halfway[1, :],
    label="Bounds from Start Point 2",
    color="r",
    linestyle="dashed",
)
ax.plot(tipsinside[0, :], tipsinside[1, :], color="b", linewidth=3)
ax.plot(
    tipsinside_halfway[0, :], tipsinside_halfway[1, :], color="r", linestyle="dashed"
)
(SP1,) = ax.plot(
    state_full[0, 0], state_full[0, 1], "bo", markersize=8, label="Start Point 1"
)
(SP2,) = ax.plot(
    state_full_halfway[0, 0],
    state_full_halfway[0, 1],
    "ro",
    markersize=8,
    label="Start Point 2",
)
...
def projection(STM_full, STT_full, state_full, J_max, dim1, dim2):
    ...
    for i in range(0, rows):
        ...
        STM = (
            STM_full[i, :, :] @ STM_full[-1, :, :] @ la.inv(STM_full[i, :, :])
        )  # STM for current timestep
        ...
        if ext1 > ext2:  # extent 1 is larger extent
            ...
            ellipse_info[i, 4] = np.arctan2(
                dir1[1], dir1[0]
            )  # rad, rotation angle from x-axis to semi-major axis
        else:  # extent 2 is larger extent
            ellipse_info[i, 2] = ext2
            ellipse_info[i, 3] = ext1
            ellipse_info[i, 4] = np.arctan2(
                dir2[1], dir2[0]
            )  # rad, rotation angle from x-axis so semi-major axis

        if ellipse_info[i, 4] < 0:
            ellipse_info[i, 4] = (
                ellipse_info[i, 4] + np.pi
            )  # correction so that ellipse rotation angle is between 0 and pi
...
def tangent_lines(ellipse_info, limits, first_axis_label, second_axis_label):
    ...
    pi = np.pi
    rotation_angle = (
        0  # rad, slope of line between ellipses' centers if attempting rotated problem
    )
    ...
    for i in range(limits[0], limits[1]):
        ...
        while (solutions_found < 2) and (k < 16) and (runthroughs < 2):
            ...
            if (last_successful == "rotated" and runthroughs == 0) or (
                last_successful == "unrotated" and runthroughs > 0
            ):
                ...
                slope_angle = np.arctan2(
                    m0, 1
                )  # radians, angle of line connecting ellipses

                supp_angle = (
                    rotation_angle - slope_angle
                )  # rotation angle that brings slope line to rotation_angle
            ...
            con1 = (
                lambda x: (
                    h1 + a1 * np.cos(x[0]) * np.cos(r1) - b1 * np.sin(x[0]) * np.sin(r1)
                )
                - x[2]
            )
            con2 = (
                lambda x: (
                    k1 + a1 * np.cos(x[0]) * np.sin(r1) + b1 * np.sin(x[0]) * np.cos(r1)
                )
                - x[4]
            )
            con3 = (
                lambda x: (
                    h2 + a2 * np.cos(x[1]) * np.cos(r2) - b2 * np.sin(x[1]) * np.sin(r2)
                )
                - x[3]
            )
            con4 = (
                lambda x: (
                    k2 + a2 * np.cos(x[1]) * np.sin(r2) + b2 * np.sin(x[1]) * np.cos(r2)
                )
                - x[5]
            )
            con5 = lambda x: x[6] * (x[3] - x[2]) - (x[5] - x[4])
            con6 = (
                lambda x: ((-b1 / a1) * (cot(x[0])) + np.tan(r1))
                / (1 + (b1 / a1) * (cot(x[0])) * np.tan(r1))
                - x[6]
            )
            con7 = (
                lambda x: ((-b2 / a2) * (cot(x[1])) + np.tan(r2))
                / (1 + (b2 / a2) * (cot(x[1])) * np.tan(r2))
                - x[6]
            )
            ...
            def objective_fun(x):
                ...
                eq6 = ((-b1 / a1) * (cot(x[0])) + np.tan(r1)) / (
                    1 + (b1 / a1) * (cot(x[0])) * np.tan(r1)
                ) - x[6]
                eq7 = ((-b2 / a2) * (cot(x[1])) + np.tan(r2)) / (
                    1 + (b2 / a2) * (cot(x[1])) * np.tan(r2)
                ) - x[6]
            ...
            )  # combinations are always generated in specific order
            ...
            while (solutions_found < 2) and (k < len(combinations)):
                ...
                m0 = (k2 - k1) / (
                    h2 - h1
                )  # initialize slope of tangent line as line connecting ellipses' centers
                # m0 = (y20 - y10) / (x20- x10)
                ...
                if d == 0 and i > limits[0] and pair_found == True:
                    ...
                    p10 = (
                        combinations[k_successful1][0][0]
                        + combinations[k_successful1][0][1]
                    ) / 2
                    p20 = (
                        combinations[k_successful1][1][0]
                        + combinations[k_successful1][1][1]
                    ) / 2
                    x10 = (
                        h1
                        + a1 * np.cos(r1) * np.cos(p10)
                        - b1 * np.sin(r1) * np.sin(p10)
                    )
                    y10 = (
                        k1
                        + a1 * np.sin(r1) * np.cos(p10)
                        + b1 * np.cos(r1) * np.sin(p10)
                    )
                    x20 = (
                        h2
                        + a2 * np.cos(r2) * np.cos(p20)
                        - b2 * np.sin(r2) * np.sin(p20)
                    )
                    y20 = (
                        k2
                        + a2 * np.sin(r2) * np.cos(p20)
                        + b2 * np.cos(r2) * np.sin(p20)
                    )
                    m0 = (k2 - k1) / (h2 - h1)
                    # m0 = (y20 - y10) / (x20 - x10)
                    ...
                    p10 = (
                        combinations[k_successful2][0][0]
                        + combinations[k_successful2][0][1]
                    ) / 2
                    p20 = (
                        combinations[k_successful2][1][0]
                        + combinations[k_successful2][1][1]
                    ) / 2
                    x10 = (
                        h1
                        + a1 * np.cos(r1) * np.cos(p10)
                ...
                if solutions_found > 0 and solution.success == True:
                    ...
                    )  # See if the second solution matches first solution. If yes, throw it away because you want two unique solutions.

                    for b in range(0, len(check)):
                        if (
                            check[b] == False
                        ):  # second solution is different from first (good)
                            matching = 0
                    ...
                    if matching == 1:
                        solution.success = (
                            False  # throw current solution away if it matches the first
                        )

                if solution.success == True:
                    solutions_found = (
                        solutions_found + 1
                    )  # add to number of unique solutions found for this version of the problem
            ...
            if (
                solutions_found == 0
            ):  # no solutions found, just say both are most recent solution
                solution1 = solution.x
                solution2 = solution1

            elif (
                solutions_found == 1
            ):  # one solution found, just say second solution is same as first
                solution2 = solution1
                ...
                if runthroughs > 0:  # attempt at original problem failed
                    ...
                    ):  # unrotated problem failed, rotated problem succeeded
                        last_successful = "rotated"  # switch last successful to rotated
                    else:
                        last_successful = (
                            "unrotated"  # switch last successful to unrotated
                        )

            # if runthroughs > 0:
            if (
                last_successful == "rotated"
            ):  # most recent success was a rotated problem, so rotate solution back
                #  ROTATE SOLUTION BACK
                x1rot1 = (
                    np.cos(supp_angle) * solution1[2]
                    + np.sin(supp_angle) * solution1[4]
                )
                y1rot1 = (
                    -np.sin(supp_angle) * solution1[2]
                    + np.cos(supp_angle) * solution1[4]
                )

                x1rot2 = (
                    np.cos(supp_angle) * solution1[3]
                    + np.sin(supp_angle) * solution1[5]
                )
                y1rot2 = (
                    -np.sin(supp_angle) * solution1[3]
                    + np.cos(supp_angle) * solution1[5]
                )

                x2rot1 = (
                    np.cos(supp_angle) * solution2[2]
                    + np.sin(supp_angle) * solution2[4]
                )
                y2rot1 = (
                    -np.sin(supp_angle) * solution2[2]
                    + np.cos(supp_angle) * solution2[4]
                )

                x2rot2 = (
                    np.cos(supp_angle) * solution2[3]
                    + np.sin(supp_angle) * solution2[5]
                )
                y2rot2 = (
                    -np.sin(supp_angle) * solution2[3]
                    + np.cos(supp_angle) * solution2[5]
                )
                ...
                solution1[4] = y1rot1
                solution1[5] = y1rot2
                solution1[6] = (solution1[5] - solution1[4]) / (
                    solution1[3] - solution1[2]
                )
                ...
                solution2[4] = y2rot1
                solution2[5] = y2rot2
                solution2[6] = (solution2[5] - solution2[4]) / (
                    solution2[3] - solution2[2]
                )
    ...
    plt.savefig(
        first_axis_label + second_axis_label + "_tangent_lines.pdf", format="pdf"
    )
    plt.show()
...
def interpolate_ellipses(ellipse_info, p1, p2):
    ...
    xs = np.linspace(
        0, rows, num=n
    )  # vector of sampled points in interpolated array (len(xs) > len(x))
    ...
    for i in range(1, rows):
        difference = (
            r[i] - r_new[i - 1]
        )  # difference between current angle and previous angle

        if difference > np.pi / 2:  # check if difference is very large
            r_new[i] = r[i] - np.pi  # unwrap angle
        elif (
            difference < -np.pi / 2
        ):  # check if difference is very large (but negative this time)
            r_new[i] = r[i] + np.pi  # unwrap angle
        else:
    ...
    (ref_trajectory,) = plt.plot(
        h, k, color=[0, 0, 0], label="Reference Trajectory"
...
if not os.path.isfile(FileName_data):
    for i in range(1000):
        ...
        scaling_factor = np.sqrt(
            2 * J_max / (np.transpose(rand_vec) @ E_star @ rand_vec)
        )
        ...
        for index in range(len(state_full)):
            dx = (
                STM_full[index, :6, :6] @ dx_0
                + STM_full[index, :6, 6:12] @ costates_initial
            )
