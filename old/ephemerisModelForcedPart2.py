...
for jj in np.arange(len(folders)):
    ...
    posCRTBP_R_dim = unitConversion.convertPos_to_dim(
        posCRTBP_R - np.array([1 - mu_cstar, 0, 0])
    ).to_value(u.km)
    ...
    for kk in np.arange(len(Isps)):
        ...
        for ii in np.arange(1, len(patchTimes) - 1):
            ...
            while not uT_flag:
                ...
                uTcheck = np.argwhere(
                    F_burn + uT_mag[ind_i:ind_f] * m_dim[ind_i:ind_f] > Ftmax
                )
                ...
                if len(uTcheck) > 0:
                    ...
                    m_ii = mi * np.exp(
                        -(dV_burn + dVCRTBPtot[ind_i]) / (Isp * g0)
                    )  # kg
                else:
        ...
        if ind_old - ind_min >= 1:
            ...
            et_patch = np.append(
                np.append(t_min, etCRTBP_mjd[ind_min + 1 : ind_old]), t_max
            )
            ...
            etCRTBP_mjd_new = np.append(
                etCRTBP_mjd_new, np.array([t_min, patchTimes[1], t_max])
            )
        ...
        for ii in np.arange(1, len(dts)):
            ...
            if ind_max - ind_min > 1:
                ...
                et_patch = np.append(
                    np.append(t_min, etCRTBP_mjd[ind_min:ind_max]), t_max
                )
                etCRTBP_mjd_new = np.append(etCRTBP_mjd_new, et_patch)
            else:
                uT_patch = uT_burn + uT_dim[ind_min, :].value
                ...
                etCRTBP_mjd_new = np.append(
                    etCRTBP_mjd_new, np.array([t_min, patchTimes[ii + 1], t_max])
                )
            uT_new = np.vstack((uT_new, uT_patch))
        ...
        times_final, states_final = ms.statePropFFIForced(
            Ts, patchStates[0, :], GM, uT_new, etCRTBP_mjd_new, omega_m
        )
        ...
        ax1.plot(
            ...
            statesR[:, 2],
            "b",
            label="Multi Segment - Ephemeris Model",
        )
        ax1.plot(
            posCRTBP_R_dim[:, 0],
            posCRTBP_R_dim[:, 1],
        ...
        interp_fc_R = RegularGridInterpolator(
            points_R, states_final_R[:, 0:3], method="linear"
        )
        ...
        dVCRTBPtotNew = (
            cumulative_trapezoid(uTNew_mag, x=etCRTBP_mjd_new, axis=0) * u.km / u.s
        )
        ...
        ax9.plot(
            timesCRTBP_d.value,
            (uT_mag * m_dim).to_value(u.mN),
            "b",
            label="Original Thrust Profile",
        )
        ax9.plot(
            uTNew_time,
            (uTNew_mag * mNew_dim).to_value(u.mN),
            "r-.",
            label="Recreated Thrust Profile",
        )
        ax9.plot(
            np.array([timesCRTBP_d[0].value, timesCRTBP_d[-1].value]),
            FtMaxPlt,
            "k",
            label="Max Thrust",
        )
        ...
        plt.plot(
            uTNew_time[:-1],
            dVCRTBPNew.to_value(u.m / u.s),
            "b",
            label="Recreated Delta-v Profile",
        )
        plt.plot(
            timesCRTBP_d[:-1].value,
            (dVCRTBP).to_value(u.m / u.s),
            "r-.",
            label="Original Delta-v Profile",
        )
        plt.scatter(
            ((patchTimes[1:-1] - patchTimes[0]) * u.s).to_value(u.d),
            dVtot,
            c="g",
            marker="*",
            zorder=3,
            label="Patch Point Burns",
        )
        plt.legend()
        plt.xlabel("time [d]")
