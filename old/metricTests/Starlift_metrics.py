            ...
            if r_mag < R_m + A:
                ...
                if r_mag * np.sin(phi) < R_m:
                    ...
                    if (tao + epsilon) > (np.pi / 2):
                        ...
                        V_spherical = ((np.pi / 3) * (R_m + A) ** 3) * (
                            4
                            - (2 + np.cos(tao)) * (1 - np.cos(tao)) ** 2
                            - (2 + np.cos(zeta)) * (1 - np.cos(zeta)) ** 2
                        )
                        ...
                        V_slit = (
                            V_spherical - V_conical
                        )  # volume of space between cone section and sphere section
                        ...
                        # find spherical cap information
                        V_spherical = ((np.pi / 3) * (R_m + A) ** 3) * (
                            (2 + np.cos(epsilon + tao))
                            * (1 - np.cos(epsilon + tao)) ** 2
                            - (2 + np.cos(tao)) * (1 - np.cos(tao)) ** 2
                        )
    ...
    def orbit_ranker(self, orbit_files, orbit_names, angle, A, metric):
        ...
        for i in range(0, len(orbit_files)):
            ...
            if metric == "orbit_volume":
                orbit = orbit_files[i]
                [time, values, r_mag_vec] = orbit.orbit_volume(
                    angle, A, "N"
                )  # obtain volumes for metric throughout orbit

            elif metric == "angular_diameter":
                orbit = orbit_files[i]
                [time, values] = orbit.angular_diameter(
                    angle, A, "N"
                )  # obtain angular diameter for metric throughout orbit
...
[avg, indices, ranking] = DRO_11.orbit_ranker(
    orbits_files, orbit_names, angle, A, "orbit_volume"
)
print(ranking)
print(avg)
print(indices)
