...
class VisibilityMetric:
    ...
    def visible_altitude(self, y_fov, z_fov, altitude):
        ...
        for x in x_from_moon:
            sc_to_moon.append(
                np.sqrt(x_from_moon[i] ** 2 + y_from_moon[i] ** 2 + z_from_moon[i] ** 2)
            )  # [km]
            i = i + 1
        ...
        volume_moon = tplquad(
            ...
            r1_moon,
            r2_moon,
            lambda r: theta1_moon,
            lambda r: theta2_moon,
            lambda r, theta: phi1_moon,
            lambda r, theta: phi2_moon,
        )[0]
        ...
        volume_alt = (
            tplquad(
                ...
                lambda r: theta1_alt,
                lambda r: theta2_alt,
                lambda r, theta: phi1_alt,
                lambda r, theta: phi2_alt,
            )[0]
        ) - volume_moon
        ...
        for x in sc_to_moon:
            if (
                sc_to_moon[i] < alt_radius
            ):  # If the spacecraft is within the target altitude
                x1, x2 = 0, abs(sc_to_moon[i] - moon_radius)
            else:  # If the spacecraft is outside the target altitude
                x1, x2 = abs(sc_to_moon[i] - alt_radius), abs(
                    sc_to_moon[i] - moon_radius
                )
            y1, y2 = (
                lambda x: -np.tan(math.radians(y_fov / 2)) * x,
                lambda x: np.tan(math.radians(y_fov / 2)) * x,
            )
            z1, z2 = (
                lambda x, y: -np.tan(math.radians(z_fov / 2)) * x,
                lambda x, y: np.tan(math.radians(z_fov / 2)) * x,
            )
            target_volume.append(
                tplquad(lambda x, y, z: 1, x1, x2, y1, y2, z1, z2)[0]
            )  # [km]
            i = i + 1
    ...
    def plot_fov(self, y_fov, z_fov, altitude, i):
        ...
        for x in x_from_moon:
            sc_to_moon.append(
                np.sqrt(x_from_moon[k] ** 2 + y_from_moon[k] ** 2 + z_from_moon[k] ** 2)
            )  # [km]
            k = k + 1
        sc_to_moon = np.array(sc_to_moon)
        ...
        x_moon = barycenter_to_moon + (
            moon_radius * np.outer(np.cos(phi), np.sin(theta))
        )
        ...
        x_line = [
            x_position[i] - sc_to_moon[i],
            x_position[i],
        ]  # "Horizontal" line from SC to moon
        y_line1 = []
        y_line2 = []
        z_line1 = []
        z_line2 = []
        ...
        for (
            ...
            x_line
        ):  # For each element in the x position vector, calculate the y and z vectors

            # Non-rotated vectors, pointed towards the negative x direction
            y_line1.append(
                (np.tan(math.radians(y_fov / 2)) * (x_line[j] - x_position[i]))
                + y_position[i]
            )
            y_line2.append(
                (-np.tan(math.radians(y_fov / 2)) * (x_line[j] - x_position[i]))
            ...
            z_line1.append(
                (np.tan(math.radians(z_fov / 2)) * (x_line[j] - x_position[i]))
                + z_position[i]
            )
            z_line2.append(
                (-np.tan(math.radians(z_fov / 2)) * (x_line[j] - x_position[i]))
        ...
        theta_fov = np.arctan(
            z_from_moon[i] / x_from_moon[i]
        )  # Rotation about the y-axis [rad]
        phi_fov = np.arctan(
            y_from_moon[i] / x_from_moon[i]
        )  # Rotation about the z-axis [rad]

        # Create the DCM
        DCM_y = np.array(
            [
                [np.cos(theta_fov), 0, -np.sin(theta_fov)],
                [0, 1, 0],
                [np.sin(theta_fov), 0, np.cos(theta_fov)],
            ]
        )
        DCM_z = np.array(
            [
                [np.cos(phi_fov), -np.sin(phi_fov), 0],
                [np.sin(phi_fov), np.cos(phi_fov), 0],
                [0, 0, 1],
            ]
        )
        ...
        fov11 = np.dot(
            ...
            np.array(
                [
                    [x_line[j] - x_position[i]],
                    [y_line1[j] - y_position[i]],
                    [z_line1[j] - z_position[i]],
                ]
            ),
        ...
        verts1 = [
            list(
                zip(
                    [x_position[i], fov11[0, 0], fov12[0, 0]],
                    [y_position[i], fov11[1, 0], fov12[1, 0]],
                    [z_position[i], fov11[2, 0], fov12[2, 0]],
                )
            )
        ]
        verts2 = [
            list(
                zip(
                    [x_position[i], fov21[0, 0], fov11[0, 0]],
                    [y_position[i], fov21[1, 0], fov11[1, 0]],
                    [z_position[i], fov21[2, 0], fov11[2, 0]],
                )
            )
        ]
        verts3 = [
            list(
                zip(
                    [x_position[i], fov22[0, 0], fov21[0, 0]],
                    [y_position[i], fov22[1, 0], fov21[1, 0]],
                    [z_position[i], fov22[2, 0], fov21[2, 0]],
                )
            )
        ]
        verts4 = [
            list(
                zip(
                    [x_position[i], fov12[0, 0], fov22[0, 0]],
                    [y_position[i], fov12[1, 0], fov22[1, 0]],
                    [z_position[i], fov12[2, 0], fov22[2, 0]],
                )
            )
        ]
