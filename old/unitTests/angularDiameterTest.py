...
def testAngularDiameter():
    ...
    if (
        ang_diam_frac[1] > 0.454
        and ang_diam_frac[1] < 0.457
        and ang_diam_frac[100] > 0.493
        and ang_diam_frac[100] < 0.496
    ):
    ...
    if ang_diam_frac[1] == 1:
        print(
            "Full view test passed"
        )  # check to see if angular diameter function works when entire view is captured
        passes = passes + 1
    else:
