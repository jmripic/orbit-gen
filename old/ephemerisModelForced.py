...
ax1.plot(
    posCRTBP_R_dim[:, 0], posCRTBP_R_dim[:, 1], posCRTBP_R_dim[:, 2], "r", label="CRTBP"
)
ax1.set_xlabel("X [km]")
ax1.set_ylabel("Y [km]")
ax1.set_zlabel("Z [km]")
...
correctedInitialEpoches, correctedInitialStates, exitflag, correctedFinalStates = (
    ms.multipleShootingIForced(
        initialEphmerisEpoches,
        initialEphemerisMCI,
        positionTolerance,
        velocityTolerance,
        GM,
...
for ii in np.arange(N - 1):
    ...
    times, states = ms.statePropFFIForced(
        Ts, correctedInitialStates[ii, :], GM, uT_dim.value, etCRTBP_mjd, omega_m
    )
...
filepath = (
    "/Users/gracegenszler/Documents/Research/starlift/orbits/forcedOrbits/" + fileStr
)
...
np.savez(
    filepath + "/InitialFF.npz",
    ICs=correctedInitialStates,
    FCs=correctedFinalStates,
    Ts=correctedInitialEpoches,
    times=timesTot,
    statesR=rotatedStates,
    statesI=inertialStates,
    Npatch=N,
    dVpatches=dVtot,
    startTime=t_start,
    mu_star=mu_cstar,
