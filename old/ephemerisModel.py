...
for kk in np.arange(0, len(Tp_target)):
    ...
    mainDir = (
        "/Users/gracegenszler/Documents/Research/starlift/orbits/forcedOrbits/naturalOrbit/"
        + str(timeDays)
        + "_days/"
    )
    ...
    while not giveUp:
        ...
        if crtbpFilePath.is_file():
            ...
            freeVar0CRTBP_R = np.array(
                [state_kk[0], state_kk[2], state_kk[4], period_kk]
            )
            ...
            posCRTBP_R_dim = unitConversion.convertPos_to_dim(
                posCRTBP_R - np.array([1 - mu_star, 0, 0])
            ).to_value(u.km)
            velCRTBP_R_dim = unitConversion.convertVel_to_dim(velCRTBP_R).to_value(
                u.km / u.s
            )
            ...
            for ii in np.arange(1, orbs):
                ...
                timesCRTBP_mjdtot = Time(
                    np.append(timesCRTBP_mjdtot.value, next_mjd.value),
                    format="mjd",
                    scale="utc",
                )
            ...
            if showPlots:
                ...
                ax2.plot(
                    posCRTBP_dimtot[:, 0], posCRTBP_dimtot[:, 1], posCRTBP_dimtot[:, 2]
                )
            ...
            np.savez(
                orbitDir + "/CRTBPDataTot.npz",
                Ts=timesCRTBP_dtot,
                Tmjd=timesCRTBP_mjdtot,
                IPs=posCRTBP_dimtot,
                IVs=velCRTBP_dimtot,
                Norbit=orbs,
            )
        ...
        if os.path.exists(initialFFFilePath):
            ...
            while exitflag != 1:
                ...
                updatedInitialEpochs, updatedInitialStates, exitflag = (
                    ms.multipleShootingI(
                        initialEphemerisEpoches, initialEphemerisMCI, posTol, velTol, GM
                    )
                )
            ...
            if exitflag == 1:
                ...
                for ii in np.arange(N1 - 1):
                    ...
                    updatedInitialStatesApoR = np.append(
                        updatedInitialStatesApoR, rStates[0, :]
                    )
                    ...
                    if showPlots:
                        ...
                        ax5.scatter(
                            states[0, 0], states[0, 1], states[0, 2], c="g", marker="o"
                        )
                        ax5.scatter(
                            states[-1, 0],
                            states[-1, 1],
                ...
                updatedInitialStatesApoR = np.append(
                    updatedInitialStatesApoR, rStates[-1, :]
                )
            ...
            if patchCtr1 > patchRound:
                ...
                np.savez(
                    orbitDir + "/ApoluneData.npz",
                    RICs=rotatedStatesApo,
                    IICs=inertialStatesApo,
                    Ts=timesFullApo,
                    patchStatesI=updatedInitialStatesApo,
                    patchTimes=updatedInitialEpochsApo,
                    patchStatesR=updatedInitialStatesApoR,
                )

        print(
            "Switching to perilune-to-perilune and minimizing the number of patch points"
        )
        ...
        if os.path.exists(periluneDataFilePath):
            ...
            # redo the patches
            timesPeri_d = (
                (timesFullApo[min1_ind:minLast_ind] - timesFullApo[min1_ind]) * u.s
            ).to("d")
            t_startStr = spice.et2utc(timesFullApo[min1_ind], "ISOC", 23, 24)
            t_startNew = Time(t_startStr, format="isot", scale="utc")
            timesPeri_mjd = Time(
                t_startNew.mjd + timesPeri_d.value, format="mjd", scale="utc"
            )
            ...
            while not minPatch:
                ...
                if plusCtr > patchRound:
                    ...
                    break
                posvel, taus = ms.getPatches(
                    N2, timesPeri_d, timesPeri_mjd, posPeri, velPeri
                )
                ...
                updatedInitialEpochs, updatedInitialStates, exitflag = (
                    ms.multipleShootingI(
                        initialEphemerisEpoches, posvel, posTol, velTol, GM
                    )
                )
                ...
                if exitflag == 1:
                    ...
                    for ii in np.arange(N2 - 1):
                        Ts = updatedInitialEpochs[ii : ii + 2]
                        times, states = ms.statePropFFI(
                            Ts, updatedInitialStates[ii, :], GM
                        )
                        ...
                        rotatedStates = np.vstack((rotatedStates, rStates))
                        timesFull = np.append(timesFull, times)
                        updatedInitialStatesPeriR = np.append(
                            updatedInitialStatesPeriR, rStates[0, :]
                        )
                    ...
                    if showPlots:
                        ...
                        plt.figure(10)
                        plt.legend(
                            loc="upper right", bbox_to_anchor=(1.4, 1), borderaxespad=0
                        )
            ...
            updatedInitialStatesPeriR = np.reshape(
                updatedInitialStatesPeriR, (N2 - 1, 6)
            )
            inertialStates = inertialStates[1:, :]
            rotatedStates = rotatedStates[1:, :]

            np.savez(
                orbitDir + "/PeriluneData.npz",
                RICs=rotatedStates,
                IICs=inertialStates,
                Ts=timesFull,
                patchStatesI=updatedInitialStates,
                patchTimes=updatedInitialEpochs,
                patchStatesR=updatedInitialStatesPeriR,
                Norbit=orbs - 1,
                Npatch=N2,
            )

        print(
            "Minimum number of patch points for "
            + str(orbs - 1)
            + " orbits: "
            + str(N2)
        )
        ...
        while oldFile and (fileCtr <= nFiles):
            if glob(eventFilePath):
                ...
                eventFilePath = (
                    orbitDir + "/attemptStartTime_" + str(t100_initial) + "*"
                )
        ...
        if not oldFile:
            ...
            while not solutionFound:
                ...
                sol_int = solve_ivp(
                    ms.ffInertial,
                    [t100_initial, t100_final],
                    state100_initial,
                    args=(GM, radii),
                    events=[ms.hitMoon, ms.hitEarth, ms.hitSun, ms.lostShape],
                    rtol=1e-12,
                    atol=1e-12,
                    method="LSODA",
                )
                ...
                if showPlots:
                    ...
                    ax16.set_title(
                        str((time100[-1] - time100[0]) / 60 / 60 / 24 / 365)
                        + " year propagation"
                    )
                ...
                if propTime > 365 * 5:
                    ...
                    if t100_ind == (len(timesFull) - 1):
                        ...
                        t_start = Time(
                            t_start.value
                            + (updatedInitialEpochs[-1] - updatedInitialEpochs[0])
                            / 60
                            / 60
                            / 24,
                            format="mjd",
            ...
            t_start = Time(
                t_start.value
                + (updatedInitialEpochs[-1] - updatedInitialEpochs[0]) / 60 / 60 / 24,
                format="mjd",
                scale="utc",
            )
