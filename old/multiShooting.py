...
def multipleShootingI(initialEpochs, initialStates, posTol, velTol, GM):
    ...
    while deltaV > velTol:
        ...
        for ii in np.arange(N - 1):
            cInitial, cFinal, STM, exitFlag1, timesTmp, statesTmp = positionShooting(
                updatedInitialEpochs[ii],
                updatedInitialStates[ii, :],
                updatedInitialEpochs[ii + 1],
                updatedInitialStates[ii + 1, :],
                posTol,
                GM,
            )
        ...
        for ii in np.arange(1, N - 1):
            ...
            a2minus = ffInertial(
                updatedInitialEpochs[ii], updatedFinalStates[ii - 1, :], GM
            )
            a2minus = a2minus[3:6]
            a2plus = ffInertial(
                updatedInitialEpochs[ii], updatedInitialStates[ii, :], GM
            )
            a2plus = a2plus[3:6]
            ...
            dVdu3 = (
                -np.linalg.inv(stm32[0:3, 3:6]) @ stm32[0:3, 0:3]
                + np.linalg.inv(stm12[0:3, 3:6]) @ stm12[0:3, 0:3]
            )
            dVdu4 = (a2plus - a2minus) + (
                np.linalg.inv(stm32[0:3, 3:6]) @ stm32[0:3, 0:3] @ v2plus
                - np.linalg.inv(stm12[0:3, 3:6]) @ stm12[0:3, 0:3] @ v2minus
            )
        ...
        updatedInitialStates[:, 0:3] = (
            updatedInitialStates[:, 0:3] + sigma * deltas[:, 0:3]
        )
        ...
        if ctrOuterLoop > maxCtrOuterLoop:
            exitflag = -1
            print(
                "Outer loop shooting exceeds maximum iteration number "
                + str(maxCtrOuterLoop)
            )
            break
...
def positionShooting(initialEpoch, initialState, targetEpoch, targetState, posTol, GM):
    ...
    while np.linalg.norm(deltaR) > posTol:
        ...
        times, states = statePropFFR(
            np.array([initialEpoch, targetEpoch]), state0, GM, omega_m
        )
...
def ffInertial(tt, w, GM, radii=None, uT=None, times=None):
    ...
    f_Earth = (
        -GM[1] * (r_sc - r_Earth) / np.linalg.norm(r_sc - r_Earth) ** 3
        - GM[1] * r_Earth / np.linalg.norm(r_Earth) ** 3
    )
    f_Sun = (
        -GM[2] * (r_sc - r_Sun) / np.linalg.norm(r_sc - r_Sun) ** 3
        - GM[2] * r_Sun / np.linalg.norm(r_Sun) ** 3
    )
...
def statePropFFI(Ts, state0, GM):
    ...
    sol_int = solve_ivp(
        ffInertial, [ti, tf], state0, args=(GM,), rtol=1e-12, atol=1e-12, method="LSODA"
    )
...
def multipleShootingIForced(
    initialEpochs, initialStates, posTol, velTol, GM, uT, timesInterp, omega_m
):
    ...
    while deltaV > velTol:
        ...
        print("Inner loop: position shooting")
        for ii in np.arange(N - 1):
            cInitial, cFinal, STM, exitFlag1 = positionShootingForced(
                updatedInitialEpochs[ii],
                updatedInitialStates[ii, :],
                updatedInitialEpochs[ii + 1],
                updatedInitialStates[ii + 1, :],
        ...
        for ii in np.arange(1, N - 1):
            ...
            a2minus = ffInertialForced(
                updatedInitialEpochs[ii],
                updatedFinalStates[ii - 1, :],
                GM,
                uT,
                timesInterp,
                omega_m,
...
def linInterp(times, uT, currentTime):
    ...
    u_current = uT[min_ind, :] + (currentTime - times[min_ind]) * (
        uT[min_ind + 1, :] - uT[min_ind, :]
    ) / (times[min_ind + 1] - times[min_ind])
