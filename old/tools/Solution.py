...
class Solution:
    ...
    def __init__(self, path_str):
            ...
            case ".p":
                ...
                with open(path_f1, "rb") as file:
                    self.data = pickle.load(
                        file
                    )  # 'data' is an attribute of the Solution class
        ...
        self.dimensioned = (
            True  # Track if the data has units/dimensions. True if they do.
        )
        pass
    ...
    def construct_vars(self):
        ...
        self.statevec = self.data[
            "state"
        ]  # statevec: position [km] and velocity [km/s]; nx6 vector
        self.tvec = self.data["t"]  # tvec: times where states are defined, [seconds]
        self.period = self.data["t"][
            -1
        ]  # period: orbit period [s]; assumes solution is ONLY one orbit
    ...
    def nondimensionalize(self, args={"redimensionalize": False}):
        ...
        }  # Default arguments. Maintains original args variable
        for key in args.keys():
            _args[key] = args[key]

        if (
            _args["redimensionalize"] == False
        ):  # Execute if no argument is passed, or if arg is passed as default
