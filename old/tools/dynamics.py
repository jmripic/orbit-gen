...
def SynodicEOMs(t, state):
    ...
    Uxx = (
        1 - (1 - mu) * (r1**2 - 3 * rb1**2) / r1**5 - mu * (r2**2 - 3 * rb2**2) / r2**5
    )
    ...
    Amat = np.concatenate(
        (np.concatenate((Z, I), axis=1), np.concatenate((U, Omega), axis=1)), axis=0
    )
...
def BarycentricEOMs(t, state):
    ...
    r1 = np.sqrt(
        (x - rb1 * np.cos(t)) ** 2 + (y - rb1 * np.sin(t)) ** 2 + z**2
    )  # Magnitude of r1-sat vector
    r2 = np.sqrt(
        (x - rb2 * np.cos(t)) ** 2 + (y - rb2 * np.sin(t)) ** 2 + z**2
    )  # Magnitude of r2-sat vector

    statedot = np.zeros(len(state))
    statedot[0:3] = state[3:6]
    statedot[3] = (
        -mu1 * (x - rb1 * np.cos(t)) / r1**3 - mu2 * (x + rb2 * np.cos(t)) / r2**3
    )
    statedot[4] = (
        -mu1 * (y - rb1 * np.sin(t)) / r1**3 - mu2 * (y - rb2 * np.sin(t)) / r2**3
    )
...
def StateTransMat(t, state, STM0):
    ...
    Uxx = (
        -((-2 * (x + mu) ** 2 + y**2 + z**2) * (1 - mu))
        / ((x + mu) ** 2 + y**2 + z**2) ** (5 / 2)
        - (mu * (y**2 + z**2 - 2 * (x + mu - 1) ** 2))
        / ((x + mu - 1) ** 2 + y**2 + z**2) ** (5 / 2)
        + 1
    )
    Uxy = (3 * y * (-mu + 1) * (mu + x)) / (
        (y**2 + z**2 + (mu + x) ** 2) ** (5 / 2)
    ) + (3 * mu * y * (mu + x - 1)) / ((y**2 + z**2 + (mu + x - 1) ** 2) ** (5 / 2))
    Uxz = (3 * z * (-mu + 1) * (mu + x)) / (
        (y**2 + z**2 + (mu + x) ** 2) ** (5 / 2)
    ) + (3 * mu * z * (mu + x - 1)) / ((y**2 + z**2 + (mu + x - 1) ** 2) ** (5 / 2))
    Uyx = (3 * y * (-mu + 1) * (mu + x)) / (
        (y**2 + z**2 + (mu + x) ** 2) ** (5 / 2)
    ) + (3 * mu * y * (mu + x - 1)) / ((y**2 + z**2 + (mu + x - 1) ** 2) ** (5 / 2))
    Uyy = (
        -(((x + mu) ** 2 + z**2 - 2 * y**2) * (1 - mu))
        / ((mu + x) ** 2 + y**2 + z**2) ** (5 / 2)
        - (mu * (-2 * y**2 + mu**2 - 2 * mu + x**2 + 2 * mu * x - 2 * x + z**2 + 1))
        / ((mu + x - 1) ** 2 + y**2 + z**2) ** (5 / 2)
        + 1
    )
    Uyz = (3 * y * z * (-mu + 1)) / ((y**2 + z**2 + (mu + x) ** 2) ** (5 / 2)) + (
        3 * mu * y * z
    ) / ((y**2 + z**2 + (mu + x - 1) ** 2) ** (5 / 2))
    Uzx = (3 * z * (-mu + 1) * (x + mu)) / (
        (y**2 + z**2 + (mu + x) ** 2) ** (5 / 2)
    ) + (3 * mu * z * (x + mu - 1)) / ((y**2 + z**2 + (mu + x - 1) ** 2) ** (5 / 2))
    Uzy = (3 * y * z * (-mu + 1)) / ((y**2 + z**2 + (mu + x) ** 2) ** (5 / 2)) + (
        3 * mu * y * z
    ) / ((y**2 + z**2 + (mu + x - 1) ** 2) ** (5 / 2))
    Uzz = -(((x + mu) ** 2 + y**2 - 2 * z**2) * (1 - mu)) / (
        (y**2 + z**2 + (mu + x) ** 2) ** (5 / 2)
    ) - (
        mu * (-2 * z**2 + mu**2 - 2 * mu + y**2 + x**2 + 2 * mu * x - 2 * x + 1)
    ) / (
        (y**2 + z**2 + (mu + x - 1) ** 2) ** (5 / 2)
    )
