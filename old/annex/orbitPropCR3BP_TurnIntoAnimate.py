...
for ii in np.arange(len(times)):
    pos_moon[ii, :] = [
        np.array(moon_r * np.cos(times[ii] + phi_moon)),
        np.array(moon_r * np.sin(times[ii] + phi_moon)),
        0,
    ]
    pos_earth[ii, :] = [
        np.array(earth_r * np.cos(times[ii] + phi_earth)),
        np.array(earth_r * np.sin(times[ii] + phi_earth)),
        0,
    ]
...
fig, ax = plot_tools.plot_bodies(
    pos_au, pos_earth, pos_moon, stk_posinert, body_names=body_names, title=title
)
...
def interpolate_positions(stk_pos, stk_times, target_times):
    ...
    interp_func_x = interp1d(
        stk_times.value, stk_pos[:, 0], kind="linear", fill_value="extrapolate"
    )
    interp_func_y = interp1d(
        stk_times.value, stk_pos[:, 1], kind="linear", fill_value="extrapolate"
    )
    interp_func_z = interp1d(
        stk_times.value, stk_pos[:, 2], kind="linear", fill_value="extrapolate"
    )
