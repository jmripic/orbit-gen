...
theta = np.arccos(
    (np.dot(moon_I_can, ideal_moon))
    / (np.linalg.norm(moon_I_can) * np.linalg.norm(ideal_moon))
)
if theta > np.pi / 2:
    theta = -theta
...
pos_dimrot = unitConversion.convertPos_to_dim(pos_canrot).to(
    "km"
)  # R frame, dimensional
vel_dimrot = unitConversion.convertVel_to_dim(vel_canrot).to(
    "km/s"
)  # R frame, dimensional
...
states_CRTBP, times_CRTBP_can = orbitEOMProp.statePropCRTBP(
    freeVar, mu_star
)  # Canonical units
...
states_FF, times_FF = orbitEOMProp.statePropFF(
    state0, t_start
)  # State is in the H frame
pos = states_FF[:, 0:3]
vel = states_FF[:, 3:6]
...
for ii in np.arange(len(times_FF_mjd)):
    pos_FF[ii, :], vel_FF[ii, :] = frameConversion.convertSC_H2I(
        pos_can[ii, :], vel_can[ii, :], times_FF_mjd[ii], C_I2G
    )
...
times = np.linspace(
    max(min(times_CRTBP_dim), min(times_FF)),
    min(max(times_CRTBP_dim), max(times_FF)),
...
for ii in np.arange(len(times_FF_mjd)):
    pos_Sun[ii, :], pos_Earth[ii, :], pos_Moon[ii, :] = frameConversion.getSunEarthMoon(
        times_mjd[ii], C_I2G
    )
    pos_Sun_H[ii, :] = (
        get_body_barycentric_posvel("Sun", times_mjd[ii])[0].get_xyz().to("AU").value
    )
    pos_Earth_H[ii, :] = (
        get_body_barycentric_posvel("Earth", times_mjd[ii])[0].get_xyz().to("AU").value
    )
    pos_Moon_H[ii, :] = (
        get_body_barycentric_posvel("Moon", times_mjd[ii])[0].get_xyz().to("AU").value
    )
...
fig_I, ax_I = plot_tools.plot_bodies(
    pos_CRTBP_interp,
    pos_FF_interp,
    pos_Earth,
    pos_Moonbody_names=body_names,
    title=title,
)
...
animate_func, ani_object = plot_tools.create_animation(
    ...
    days,
    desired_duration,
    [pos_CRTBP_interp, pos_FF_interp, pos_Earth, pos_Moon],
