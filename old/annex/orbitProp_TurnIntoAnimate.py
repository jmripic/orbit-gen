...
print("Dimensional [km] position IC in the rotating frame: " + str(pos_dim))
print("Dimensional [km/s] velocity IC in the rotating frame: " + str(vel_dim))
...
freeVar_CRTBP_R = np.array([IC[0], IC[2], IC[4], 1 * IC[-1]])
...
# Propagate the dynamics in the CRTBP model for 1 orbit
statesCRTBP_R, timesCRTBP_R = orbitEOMProp.statePropCRTBP_R(freeVar_CRTBP_R, mu_star)
posCRTBP_R = statesCRTBP_R[:, 0:3]
posCRTBP_R = unitConversion.convertPos_to_dim(posCRTBP_R).to("AU")
...
for kk in np.arange(len(timesCRTBP_mjd)):
    ...
    r_PO_H, _ = frameConversion.convertSC_I2H(
        posCRTBP[kk, :], velCRTBP[kk, :], time, C_I2G
    )
...
ax1.plot(posCRTBP_R[:, 0], posCRTBP_R[:, 1], posCRTBP_R[:, 2], "b", label="CRTBP R")
ax1.plot(
    r_CRTBP_rot[:, 0], r_CRTBP_rot[:, 1], r_CRTBP_rot[:, 2], "r-.", label="CRTBP I2R"
)
plt.show()
breakpoint()
...
state0 = np.append(
    np.append(pos_H.value, vel_H.value), 33 * times_dim[-1].value
)  # Change to Tp_dim.value for one orbit
...
for ii in np.arange(len(timesFF_mjd)):
    ...
    r_EMO = (
        get_body_barycentric_posvel("Earth-Moon-Barycenter", time)[0].get_xyz().to("AU")
    )
...
ax1.scatter(
    posFF[0, 0], posFF[0, 1], posFF[0, 2], c="b", marker="*", label="Full Force Start"
)
ax1.scatter(
    posFF[-1, 0], posFF[-1, 1], posFF[-1, 2], c="b", marker="D", label="Full Force End"
)
ax1.scatter(
    r_PO_CRTBP[0, 0],
    r_PO_CRTBP[0, 1],
...
animate_func, ani_object = plot_tools.create_animation(
    timesFF,
    33 * IC[-1],
    desired_duration,
    [r_PEM_i, r_EarthEM_i, r_MoonEM_i, r_SunEM_i],
...
animate_func2, ani_object2 = plot_tools.create_animation(
    timesFF,
    33 * IC[-1],
    desired_duration,
    [r_PEM_i, r_EarthEM_i, r_MoonEM_i],
    body_names=body_names,
    title=title,
)
# Save
