...
def statePropFF(freeVar, t_mjd, mu_star, C_G2I):
    ...
    sol_int = solve_ivp(
        FF_EOM, [0, T], freeVar[0:6], args=(t_mjd, mu_star, C_G2I), method="LSODA"
    )
...
def calcMonodromyMatrix(freeVar, mu_star, m1, m2):
    ...
    dydy = (
        1 - m1 / rp1**3 - m2 / rp2**3 + 3 * m1 * y**2 / rp1**5 + 3 * m2 * y**2 / rp2**5
    )
    dydz = 3 * m1 * y * z / rp1**5 + 3 * m2 * y * z / rp2**5
    dzdz = (
        1 - m1 / rp1**3 - m2 / rp2**3 + 3 * m1 * z**2 / rp1**5 + 3 * m2 * z**2 / rp2**5
    )
...
vI = frameConversion.rot2inertV(np.array(IC[0:3]), np.array(IC[3:6]), 0)
v_EMO = (
    get_body_barycentric_posvel("Earth-Moon-Barycenter", t_mjd)[1]
    .get_xyz()
    .to("AU/day")
)
...
state0 = np.array(
    [pos_ICRS[0], pos_ICRS[1], pos_ICRS[2], vel_ICRS[0], vel_ICRS[1], vel_ICRS[2], 6]
)  # 124*Tp_dim
